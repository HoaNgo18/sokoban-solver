...
def BFSsolution(game):
    ...
    print("BFS Algorithm processing...")
...
def display_box(screen, message):
    "Print a message in a box in the middle of the screen, scaled up by 2x"
    fontobject = pygame.font.Font(None, 36)  # Font size doubled
    pygame.draw.rect(screen, (0, 0, 0),
                     ((screen.get_width() / 2) - 200,
                      (screen.get_height() / 2) - 20,
                      400, 40), 0)  # Box size doubled
    pygame.draw.rect(screen, (255, 255, 255),
                     ((screen.get_width() / 2) - 202,
                      (screen.get_height() / 2) - 22,
                      404, 44), 1)  # Border size doubled
    if len(message) != 0:
        screen.blit(fontobject.render(message, 1, (255, 255, 255)),
                    ((screen.get_width() / 2) - 200, (screen.get_height() / 2) - 20))  # Text position adjusted
    pygame.display.flip()
...
def start_game():
    start = pygame.display.set_mode((640,480))
    level = ask(start,"Select Level (1 - 50) ")
    if int (level) > 0:
        return level
    else:
...
box = pygame.image.load('.\images/boxOff.png')
box_docked = pygame.image.load('.\images/boxOn.png')
front = pygame.image.load('.\images\\front.png')
worker_docked = pygame.image.load('.\images\\front.png')
docker = pygame.image.load('.\images\spot.png')
...
while 1:
    ...
    if sol == "NoSol":
        ...
        time.sleep(1.5)  # Cho người dùng thời gian xem thông báo
        level = start_game()
        game = Game(map_open('./levels', level))
        size = game.load_size()
        screen = pygame.display.set_mode(size)
        ...
        i = 0
        flagAuto = 0
        continue  # Quay lại vòng lặp để chơi cấp độ mới
    if sol == "TimeOut":
